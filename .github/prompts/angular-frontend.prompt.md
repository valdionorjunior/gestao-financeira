...
## Dicas de UI/UX para Finanças

### Regra 60-30-10
Distribua as cores seguindo a proporção:
- **60%** → cor neutra (fundo da página, `--color-bg`)
- **30%** → cor secundária (cards, sidebars, `--color-surface`)
- **10%** → cor de destaque (CTAs, botões primários, `--color-primary`)

### Verde e Vermelho com Acessibilidade
- Use **verde** para valores positivos (receitas, saldos favoráveis) e **vermelho** para negativos (despesas, alertas).
- Para usuários com daltonismo, prefira **tons azulados** (`#0ea5e9`) no lugar de verde puro em indicadores críticos.
- Nunca transmita informação **apenas pela cor** — combine com ícones (`pi-arrow-up`, `pi-arrow-down`) e rótulos textuais.

### Dark Mode
- Fundamental para reduzir a fadiga visual de usuários que monitoram finanças ao longo do dia.
- Implemente via classe `.dark` no `<html>`, conforme configurado em `darkModeSelector` do PrimeNG.
- Garanta que o contraste de texto atinja no mínimo **4.5:1** (WCAG AA) nos dois temas.

### Ferramentas de Referência
- [Coolors](https://coolors.co) — geração de paletas harmônicas
- [Adobe Color](https://color.adobe.com) — harmonização e verificação de acessibilidade
- [Who Can Use](https://www.whocanuse.com) — simulação de daltonismo para as cores escolhidas

### Variáveis CSS
Use exclusivamente as CSS custom properties definidas em `styles.css` para todas as cores, nunca valores hexadecimais hardcoded nos componentes:
...
<!-- ❌ Evitar -->
<div class="bg-[#1C1F22] text-[#FFFFFF]">
```

Isso garante que a troca de tema claro/escuro funcione automaticamente em toda a aplicação.
